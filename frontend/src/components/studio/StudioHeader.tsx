...
export function StudioHeader({ active }: { active: "studio" | "voice" | "calls" | "runs" }) {
  const tabs = [
    { key: "studio", label: "Studio", href: "/studio" },
    { key: "voice", label: "Voice test", href: "/studio/voice" },
    { key: "calls", label: "Calls", href: "/studio/calls" },
    ...
    <header className="sticky top-0 z-30 border-b border-border bg-background/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-6">
        <div className="flex min-w-0 items-center gap-4 sm:gap-8">
          ...
          <nav className="flex min-w-0 items-center gap-4 overflow-x-auto text-sm sm:gap-5">
            {tabs.map((t) => (
              <Link
                key={t.key}
                href={t.href}
