...
function CallCard({
  ...
  const footer = [
    call.metadata.mode === "rehearsal" || call.metadata.calendar === "sandbox" ? "Rehearsal" : "Live",
    ago(call.started_at),
...
export function CallsView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (requested) setSelected(await api.getConversation(requested));
      else if (rows.length > 0) setSelected(await api.getConversation(rows[0].conversation_id));
    } catch (cause) {
    ...
    <div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <p className="max-w-2xl text-sm text-muted-foreground">
          Every conversation the voice agent held, with what it said and what it actually did. The
          outcome is the engine&rsquo;s ruling on the governed write, not the agent&rsquo;s wording.
        </p>
        <button
          type="button"
          onClick={() => void load()}
