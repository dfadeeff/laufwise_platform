...
@dataclass(frozen=True)
class BusyRange:
    """One treatment room taken between two instants — the reverse direction (ADR-0009).
...
def busy_digest(ranges: Iterable[BusyRange]) -> tuple[tuple[str, str, str, str], ...]:
    ...
    being equal is what "the website shows what thevea has" means (ADR-0009 D1)."""
    return tuple(
        sorted(
...
class OccupancySource(Protocol):
    """The read side of the mirror (ADR-0009 D2): when is each room taken? Separate from
    `DestinationCalendar` and from `PracticeCalendar` on purpose — a mirror run is handed this
    capability only, so it physically cannot write into the practice calendar it is reading."""
...
class AvailabilityMirror(Protocol):
    """The write side of the mirror (ADR-0009 D2): the booking site's copy of that occupancy."""
